...
flowchart TD
    A["GitHub Projects\nイシュー作成"] --> B["EARS 要件定義\nspecs/ears/*.md"]
    B --> C["Gherkin シナリオ\nspecs/gherkin/*.feature"]
    C --> D["仕様駆動開発\n実装（Server Component 優先）\n@see req-XXX / uc-XXX を付与"]
    D --> E["単体テスト\nVitest"]
    E --> F["結合テスト\nVitest + Testing Library + MSW"]
    F --> G["E2E テスト\nPlaywright"]
    G --> H{"CI パス"}
    H -- "Yes" --> I["PR 作成・レビュー・マージ"]
    H -- "No" --> D
    I --> J["Vercel 自動デプロイ"]
```

> **Specs First**: EARS 要件と Gherkin シナリオが存在しない機能の実装は禁止。
> 仕様 ID（`req-XXX`, `uc-XXX`）はコード・テスト名に必ず紐付けること。

### タスク管理・意思決定（GitHub Projects）

| 用途                          | 場所                                 |
| ----------------------------- | ------------------------------------ |
| タスク管理                    | GitHub Projects（カンバンボード）    |
| バグ・機能 Issue              | GitHub Issues                        |
| アーキテクチャ意思決定（ADR） | GitHub Discussions（`ADR` カテゴリ） |

- Issue には設計メモ・決定理由・関連する仕様 ID をコメントとして残す
- ADR は「なぜその設計を選んだか」を Discussions に記録し、後から辿れるようにする
- Issue 番号はコミットメッセージ・ブランチ名に含め、コードと一本のスレッドで追跡できる状態を保つ
